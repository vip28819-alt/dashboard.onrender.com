...
export function startDashboard({ client, getGuildData, saveData, createTicketSetup, createServerSetup, ensurePrivateLogChannel, publishVerificationPanel, getAuthenticHadith }) {
  ...
  app.patch('/api/guilds/:id', (request, response) => {
    ...
    if (!guild) return response.status(404).json({ error: 'Server not found.' });
    const settings = getGuildData(guild.id);
    for (const key of ['prefix', 'language', 'embedColor', 'commandEnabled', 'commandAliases', 'autoRoleIds', 'selfAssignableRoleIds', 'autoReplies', 'quarantineRoleId', 'verifiedRoleId', 'joinToCreateChannelId', 'joinToCreateControlChannelId', 'afkRoleId', 'afkChannelId', 'suggestionsChannelId', 'islamicReminders', 'welcomeChannelId', 'welcomeMessage', 'welcomeDelivery', 'welcomeCardEnabled', 'welcomeImageUrl', 'verificationChannelId', 'verificationEnabled', 'goodbyeChannelId', 'goodbyeMessage', 'logChannelId', 'logCategoryId', 'logEvents', 'rulesChannelId', 'rulesText', 'ticketCategoryId', 'ticketPanelChannelId', 'automod', 'automodAction', 'levelUp', 'levelUpMessage', 'levelExcludedChannels', 'levelExcludedRoles', 'levelRoleRewards', 'blockedWords', 'ticketPanelTitle', 'ticketPanelDescription', 'ticketButtonLabel', 'ticketButtonStyle', 'ticketNamePrefix', 'ticketWelcomeTitle', 'ticketWelcomeMessage', 'ticketCloseLabel']) if (request.body[key] !== undefined) settings[key] = request.body[key];
    if (request.body.welcomeImageUrl !== undefined && request.body.welcomeImageUrl !== null && request.body.welcomeImageUrl !== '' && !/^https?:\/\/\S+$/i.test(String(request.body.welcomeImageUrl).trim())) return response.status(400).json({ error: 'Welcome image URL must use http:// or https://.' });
    ...
    const validChannelIds = new Set(guild.channels.cache.filter((channel) => channel.isTextBased() || [ChannelType.GuildCategory, ChannelType.GuildVoice, ChannelType.GuildStageVoice].includes(channel.type)).keys());
    const validRoleIds = new Set(guild.roles.cache.filter((role) => role.id !== guild.id).keys());
    const channelKeys = ['welcomeChannelId', 'goodbyeChannelId', 'verificationChannelId', 'joinToCreateChannelId', 'joinToCreateControlChannelId', 'afkChannelId', 'suggestionsChannelId', 'logChannelId', 'logCategoryId', 'rulesChannelId', 'ticketCategoryId', 'ticketPanelChannelId'];
